...
- **Mechanistic confirmation.** We confirm the predicted mechanism in two stages. An idealised probit simulation under assumptions (A1)--(A2) reproduces the $\eta$--$I(X;Y)$ mapping and the efficiency--power tension in a controlled setting ((sec:results); Supplementary (sec:si_note_s2)). Four single-KPI exemplars, one per quadrant, then test the taxonomy's directional predictions in real data when signal strength ($\delta/\sigma_A$) is held comparable ((fig:pef_ml,tab:exemplars)). The exemplars are a directional check at matched signal, not a claim of large Quadrant 4 predictive gain. Multi-KPI feature interactions are identified as a direct extension ((sec:discussion)).
