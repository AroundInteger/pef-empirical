...
function figure_3_ml_mapping(ml_all, ~, fpath)
    ...
    % Panel (B) y-axis starts at 0 (Q1 mean rounds to -0.0%; no empty negative floor).
    ...
    % Offsets chosen to sit in empty pockets (not on the Q3 cloud or each other).
    ...
    for ei = 1:4
        ...
        text(xe + exemplar_xoff(ei), ye + exemplar_yoff(ei), exemplar_label(ei), ...
            'FontSize', ST.fs_annot, 'FontWeight', 'bold', 'Color', [0.10 0.10 0.10], ...
    ...
    if n_clip_x + n_clip_y > 0
        text(ax1, 0.98, 0.02, sprintf('%d pt(s) outside shown range', n_clip_x+n_clip_y), ...
