...
After an empirical pipeline run that changes geometry/ψ outputs:

```bash
EMPIRICAL=/Users/rowanbrown/Documents/GitHub/pef-empirical
MATH=/Users/rowanbrown/Documents/GitHub/pef-mathematics
cp "$EMPIRICAL"/scripts/paper_pipeline/outputs/{kappa_symmetry_*,psi_*,pef_landscape_2season_geometry.csv,domain_summary.csv,table_numbers.csv} \
   "$MATH"/validation_inputs/
```
...
- Module C: idealised simulation aligned to (κ, ρ, δ, probit) spec (discussion only so far)
