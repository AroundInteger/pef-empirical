...
\begin{figure}[p]
  ...
  \caption{Observed machine learning improvement $\Delta\mathrm{ML}$ (relative minus absolute features, five-fold CV logistic) versus $\hat\eta$ across the full sports KPI inventory ($N=\PEFtotalStudies$; landscape characterisation, \cref{sec:outcome_defs}). Rugby URC: circles; football Championship: squares; points coloured by quadrant. The inventory is Q3-dominant (68\% of KPIs). Four annotated exemplars (one per quadrant; \cref{tab:exemplars}), selected at comparable $\delta/\sigma_{\mathrm{A}}$ values ($0.16$--$0.32$), provide the mechanistic confirmation; per-KPI detail in Supplementary Figures~S2--S3. See \cref{sec:signal_strength} for signal strength as an independent moderator.}
  \label{fig:pef_ml}
\end{figure}
