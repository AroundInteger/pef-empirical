...
\begin{figure}[p]
  ...
  \caption{PEF landscape: $\eta$ as a function of $\rho$ and $\kappa$, with four quadrants labelled.
    ...
    the admissibility blow-up as $\rho\to +1$ (Supplementary~\cref{sec:si_note_s3}) remains visible; contour
...
\begin{figure}[p]
  ...
  \caption{Observed machine learning improvement $\Delta\mathrm{ML}$ (relative minus absolute features, team-blocked five-fold CV logistic; \cref{sec:ml_cv}) versus $\hat\eta$ across the full sports KPI inventory ($N=\PEFtotalStudies$; landscape characterisation, \cref{sec:outcome_defs}).
    (A)~Per-KPI scatter with annotated quadrant exemplars and the high-$\eta$ rucks-won counter-example.
    (B)~Quadrant means with \emph{upper} 1\,SE only (within-quadrant $\Delta\mathrm{ML}$ is highly heterogeneous; SE summarises inventory spread, not confirmatory uncertainty).
    Rugby URC and football Championship points are coloured by quadrant. The inventory is Q3-dominant ($\PEFlandscapeQthreePct\%$ of KPIs).
    Four exemplars at comparable $\delta/\sigma_{\mathrm{A}}$ (\cref{tab:exemplars}) provide a directional check at matched signal, not a large Q4 gain. Relative improvement despite $\eta<1$ is visible in the scatter. Per-KPI detail in Supplementary~\cref{fig:si_kpi_labelled,fig:si_ipred_vs_dml}.
    Fisher--Rao $\psi$-scale diagnostics are developed in the companion mathematics paper \parencite{brownPEFmath}.}
  \label{fig:pef_ml}
\end{figure}
