...
\begin{figure}[p]
  ...
  \caption{Observed machine learning improvement $\Delta\mathrm{ML}$ (relative minus absolute features, team-blocked five-fold CV logistic; \cref{sec:ml_cv}) versus $\hat\eta$ across the full sports KPI inventory ($N=\PEFtotalStudies$; landscape characterisation, \cref{sec:outcome_defs}). Rugby URC: circles; football Championship: squares; points coloured by quadrant. The inventory is Q3-dominant ($\PEFlandscapeQthreePct\%$ of KPIs). Four annotated exemplars (one per quadrant; \cref{tab:exemplars}), selected at comparable $\delta/\sigma_{\mathrm{A}}$ values ($\PEFexDeltaRatioLo$--$\PEFexDeltaRatioHi$), provide the mechanistic confirmation; per-KPI detail in Supplementary~\cref{fig:si_kpi_labelled,fig:si_ipred_vs_dml}. See \cref{sec:signal_strength} for signal strength as an independent moderator. Fisher--Rao $\psi$-scale diagnostics and regime stratification of ML residuals are developed in the companion mathematics paper \parencite{brownPEFmath}.}
  \label{fig:pef_ml}
\end{figure}
