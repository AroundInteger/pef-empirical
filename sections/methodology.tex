...
To assess the PEF-to-ML mapping, logistic regression was fitted with five-fold cross-validation \parencite{kohavi1995}, implemented in {MATLAB} (\texttt{fitglm}, Statistics and Machine Learning Toolbox). Performance improvement was quantified as the accuracy gain from relative features ($X_{\mathrm{A}}-X_{\mathrm{B}}$) versus absolute features ($X_{\mathrm{A}}$ only).

\subsection{Statistical Rigour}

\textbf{Multiple comparison corrections.} Where hypothesis tests were applied across a fixed battery of sports KPI checks, three methods were considered: Bonferroni correction (family-wise error rate at $\alpha=0.05$), Benjamini--Hochberg FDR control \parencite{benjamini1995}, and Holm--Bonferroni step-down procedure \parencite{holm1979}. \Cref{tab:mc} summarises an illustrative legacy tabulation; the primary sports analysis reports per-KPI bootstrap intervals (\S\ref{sec:results}, Supplementary Information).
