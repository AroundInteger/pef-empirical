...
\begin{equation}
  ...
  \label{eq:hygx}
\end{equation}
where $H(p)=-p\log_{2}p-(1-p)\log_{2}(1-p)$ is the binary entropy function. (Full derivation in \cref{sec:appendix}.) The PEF formula rearranges to $\Var(X)=\sigma^{2}_{\mathrm{A}}(1+\kappa)/\eta$, linking variance geometry directly to $\eta$. Substituting into \cref{eq:hygx} yields
\begin{equation}
