...
This has a practical consequence: pooling KPIs with heterogeneous signal strengths weakens the observed $\eta$--$\Delta\mathrm{ML}$ relationship. Across \PEFtotalStudies{} sports KPI studies, $\delta/\sigma_{\mathrm{A}}$ varies substantially (median $\PEFmedianDeltaRatio$; Supplementary~\cref{fig:si_iso_eta_I}), and the pooled $\Corr(\eta,\Delta\mathrm{ML})=\PEFcorrEtaML$ is accordingly weak. This is the expected behaviour under the theory, not a failure of it: $\eta$ encodes the $(\kappa,\rho)$ position, but $\delta/\sigma_{\mathrm{A}}$ is an independent source of variation in ML outcome that must be accounted for separately.
