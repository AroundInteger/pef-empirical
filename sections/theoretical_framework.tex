...
\begin{enumerate}
  ...
  \item the empirical PEF-to-ML mapping (\cref{eq:dml_poly}) and cross-domain validation;
...
\subsection{Empirical PEF-to-ML Mapping}

Beyond qualitative guidance, we seek a quantitative relationship between $\eta$ and observed machine learning improvement. Fitting a second-order polynomial to cross-domain validation data yields
\begin{equation}
  \Delta\mathrm{ML} = 0.234(\eta-1) + 0.089(\eta-1)^{2}.
  \label{eq:dml_poly}
\end{equation}

\noindent Here $\Delta\mathrm{ML}$ is the proportional improvement in classification accuracy from using relative features ($X_{\mathrm{A}}-X_{\mathrm{B}}$) versus absolute features ($X_{\mathrm{A}}$, $X_{\mathrm{B}}$), $\eta$ is the Paired Efficiency Factor (\cref{eq:pef}), and the coefficients $0.234$ and $0.089$ are empirically fitted to the cross-domain validation data (\cref{tab:pef_ml}). This is an \textbf{empirical} relationship, not a theoretical derivation; it provides first-order estimates of expected performance gains given estimated $\eta$. Validation is presented in \cref{sec:results}.
