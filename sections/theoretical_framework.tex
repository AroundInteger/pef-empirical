...
\end{equation*}
recovering the paired-efficiency formula from the Introduction \parencite{fisher1935}. In words, positive correlation shrinks the variance of the difference below the independent-measurement baseline ($\eta>1$), whilst negative correlation amplifies it ($\eta<1$). This is exactly the efficiency logic of the paired $t$-test. The PEF formula therefore \emph{contains} Fisher's result as a special case; unequal variances ($\kappa\neq 1$) are the ingredient the classical framework cannot represent on its own.
