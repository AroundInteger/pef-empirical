%% SI Note S3 (mathematical derivations). Input from sections/supplementary.tex (SI Section 2).
\subsection*{Supplementary Note S3: Mathematical Derivations}
\label{sec:si_note_s3}
\label{sec:appendix}

\noindent The algebraic derivation of the PEF formula (\cref{eq:pef}) from the variance of correlated differences, together with its reduction to Fisher's classical case at $\kappa=1$, is given in full in \cref{sec:theory} (\S2.1). This note records the material not reproduced in the main text: the step-by-step information content derivation, the boundary analysis, the statistical properties of the PEF estimator, and the Pitman asymptotic relative efficiency proof.

\subsubsection*{Information Content Derivation}
...
\paragraph*{Fisher Regime ($\kappa=1$, $\rho=0$)}
...
\paragraph*{Variance Ratio Extremities}
...
\paragraph*{Correlation Extremities}
...
\paragraph*{Summary of Special Cases}
...
\subsubsection*{Statistical Properties of the PEF Estimator}
...
\subsubsection*{Connection to Classical Tests}
...
\subsubsection*{Connection to Pitman Asymptotic Relative Efficiency}
\label{sec:pitman}
